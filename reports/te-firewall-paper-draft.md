...
**Working draft — v0.5 (adds delay-resolved TE §3.8/§4.5, Fig 6, Table 5).** Target: ocean/wind engineering journal (Ocean Engineering / Renewable Energy / Wind Energy Science tier). Citation style: APA 7.0.
...
Transfer entropy carries information the undirected methods do not: the *delay* at which a source acts on a target. Table 5 reports the selected coupling delay — the lag maximising the delay-resolved TE (§3.8) — for the significant wave-driven edges, averaged over three healthy 11 m/s seeds. The delays are physically sensible and channel-specific: platform pitch responds to wave forcing almost immediately (≈ 0.3 s), consistent with a near-instantaneous overturning moment, while heave (≈ 2.7 s), the fairlead mooring tension (≈ 3.9 s), and surge (≈ 4.3 s) lag progressively, tracking the slower heave-restoring and mooring-mediated response. These timescales are recoverable only because the method is directed and lag-resolved; a coherence peak carries no such information.

**Table 5.** Selected wave-coupling delay (the lag maximising the delay-resolved TE, §3.8) for the significant wave edges, mean over three healthy 11 m/s seeds.

| Edge | Selected delay (s) |
|---|---|
| Wave → PtfmPitch | 0.3 |
| Wave → PtfmHeave | 2.7 |
| Wave → FAIRTEN2 | 3.9 |
