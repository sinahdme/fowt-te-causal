...
1)  Clone the openfast repo and initialize the testing database
    ...
    >
    > 4)  Type: `cd openfast` (change your working directory to the openfast folder)
    > 5)  Type: `git checkout dev` (this places your local repo on the correct branch of the openfast repo)
    > 6)  Type: `git submodule update --init --recursive` (this downloads the testing database to your computer) You should see something like this:
    >
    > <!-- -->
    >
...
5)  Execute the OpenFAST regression Tests
    ...
    >
    > 5)  If an individual test succeeds you will see `PASS` otherwise you will see `FAIL` after that test's name
