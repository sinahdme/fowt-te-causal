...
With `GENERATE_TYPES` enabled, CMake will configure the <span class="title-ref">openfast-registry</span> target to compile as a dependency of all other targets. The OpenFAST Registry executable will be found in `openfast/build/modules/openfast-registry/openfast-registry`.
...
The process for adding a new types module follows `regenerating_types` closely. Here, an additional step is required to configure CMake to execute the Registry on the new input file and include the resulting types module in the compile step.
