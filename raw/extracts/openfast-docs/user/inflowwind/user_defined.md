...
- See the original `InflowWind Manual <InflowWind_Manual.pdf>` for general InflowWind information
- Review existing wind field implementations (Uniform, Grid3D) in the source code for reference
- Check `ifw_appendix` for example input files
- Refer to NWTC Library documentation for array allocation and error handling utilities
- See `ifw_angles` for information about wind coordinate systems and rotations
