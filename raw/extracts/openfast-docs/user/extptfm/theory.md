...
This section focuses on the theory behind the ExtPtfm module. The theory was published in the following article ep-Branlard:2020superelement ([access article here](https://iopscience.iop.org/article/10.1088/1742-6596/1452/1/012033)) which may be used to reference this documention.

ExtPtfm relies on a dynamics system reduction via the Craig-Bampton (C-B) method ep-CraigBampton:1968.
...
Eq. `FollowMotion` provides the motion of the followers as a function of the leaders’ motion under the assumptions of the Guyan reduction ep-Guyan:1965.
...
<figure>
<img src="figs/MasterDOFScheme.png" alt="figs/MasterDOFScheme.png" />
</figure>
...
Eq. `moduleB` is rewritten in the state-space form of Eq. `StateSpaceForm` as follows. The second block row of `moduleB` is developed to isolate $`\boldsymbol{\ddot{x}}_2`$. Using $`\boldsymbol{M}_{r22}=\boldsymbol{I}`$ and reintroducing the damping matrix for completeness gives:
...
The matrices of the state-space relation from Eq. `StateSpaceForm` are then directly identified as (ep-Branlard:2020superelement):
...
Isolating $`\boldsymbol{f}_{C}`$ from the first block row of Eq. `moduleB` and using the expression of $`\boldsymbol{\ddot{x}}_2`$ from Eq. `xddot2` leads to:
...
The matrices of for the output $`\boldsymbol{y}`$ are then identified as (ep-Branlard:2020superelement):
