...
- Support inflow-, and wake-related output channels for more than the first 9 wind turbines in the wind farm.
- Interface FAST.Farm to the Wind-Plant Integrated System Design & Engineering Model ([WISDEM](https://github.com/NatLabRockies/WISDEM)$`^\text{TM}`$) for systems-engineering applications (multidisciplinary design, analysis, and optimization; uncertainty quantification; and so on).
