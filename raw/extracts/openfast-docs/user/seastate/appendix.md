...
The following is a SeaState primary input file generating irregular (JONSWAP) waves internally structure:
...
The following is a SeaState driver input file structure:
...
This is a list of all possible output channels for the SeaState module. The names are grouped by meaning, but can be ordered in the OUTPUT CHANNELS section of the primary SeaState input file as you see fit. α refers to the output position for either wave elevation or wave kinematics specified in the OUTPUT section of the primary SeaState input file, where α is a number in the range \[1,NWaveElev\] for wave elevation outputs and in the range \[1,NWaveKin\] for wave kinematics outputs. Setting α \> NWaveElev or α \> NWaveKin yields invalid output. All outputs are in the global inertial-frame coordinate system.
...
| FAccαxi, FAccαyi, FAccαzi | (m/s<sup>2</sup>), (m/s<sup>2</sup>), (m/s<sup>2</sup>) | Total (first- plus second-order waves) fluid accelerations at α |
| FDynPα | (Pa) | Total (first- plus second-order waves) fluid dynamic pressure at α |
| FAccMCFαxi, FAccMCFαyi, FAccMCFαzi | (m/s<sup>2</sup>), (m/s<sup>2</sup>), (m/s<sup>2</sup>) | Scaled first-order-wave fluid accelerations for the MacCamy-Fuchs members in HydroDyn at α |
